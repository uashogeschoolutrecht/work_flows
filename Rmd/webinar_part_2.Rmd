---
title: "Part 2; Managing your (data) projects with 'Guerilla Analytics'"
...
MD5SUMS are
 ...
 >- A unique code to identify a file (`r tools::md5sum(here::here("Rmd", "webinar_part_2.html"))`)
 ...
 >- If you sent a data file, sent the md5sums along
 >- md5sums are also used for safety: checking an md5sum ensures that the code is valid and has not changed
 
## How does that look in practice? {.build}

```{r}
head(readr::read_csv(here::here("data", "D020", "tidy_excel.csv")), 1)
```
```{r, echo=TRUE}
x <- readr::read_lines(here::here("data", "D020", "tidy_excel.csv.md5")); x
y <- tools::md5sum(here::here("data", "D020", "tidy_excel.csv")) %>% unname(); y
```
```{r, echo=TRUE}
x == y
```
...
## Sharing data
 ...
 >- Suppress the impulse to store multiple copies of the data in different locations (cloud and offline is a good idea, synchronizing is a bad idea)
